...
const ROLES = ["Frontend Developer", "Systems Engineer", "Mobile Developer", "React Specialist"];
      ...
      const scramble = setInterval(() => {
        setDisplay(
          target.split("").map((char, i) => {
            if (char === " ") return " ";
            if (i < Math.floor((iteration / totalFrames) * target.length)) return char;
...
export function HeroSection() {
  ...
  return (
    <section
      ref={sectionRef}
      id="hero"
      className="relative min-h-screen flex items-center border-b"
    >
      <motion.div
        style={{ opacity, y }}
      ...
      >
        ...
        >
          ...
          <motion.div variants={fadeUp} className=" h-10 flex items-center">
            <span className={`font-mono text-xl md:text-2xl transition-colors duration-200 ${
          ...
          <motion.p
            variants={fadeUp}
            className="text-lg md:text-xl text-text-secondary font-light mb-4 max-w-xl leading-relaxed"
          >
            Apasionado por crear{" "}
            <span className="text-text-primary font-medium">interfaces que se sienten vivas</span>.
          </motion.p>
