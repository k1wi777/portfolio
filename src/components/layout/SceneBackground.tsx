...
function MouseParticles() {
  ...
  return (
    <points ref={pointsRef}>
      ...
      <pointsMaterial size={0.06} color="#00f5ff" transparent opacity={0.5} sizeAttenuation depthWrite={false} />
    </points>
  );
}
...
function ScrollSphere({ scrollProgress }: { scrollProgress: number }) {
  ...
  useFrame((state) => {
    ...
    // 0%  → centrado a la derecha
    // 25% → centro pantalla, más grande
    // 50% → izquierda, achicándose
    // 75% → abajo a la derecha, pequeño
    // 100%→ funde / casi invisible
    ...
    // Color dinámico según scroll
    // 0% cyan, 50% azul, 100% purple
    if (mat?.color) {
      const h = 185 + sp * 95; // 185 (cyan) → 280 (purple)
  ...
  return (
    <group ref={groupRef} position={[2.5, 0, 0]}>
      ...
      <mesh ref={ring2} rotation={[Math.PI / 3.5, 1.0, 0]}>
        ...
        <meshBasicMaterial color="#bf5af2" transparent opacity={0.3} />
      </mesh>
...
function Scene({ scrollProgress }: { scrollProgress: number }) {
  return (
    <>
      ...
      <Stars radius={100} depth={60} count={4000} factor={3} saturation={0.3} fade speed={0.3} />
